...
export default function BookingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchAvailability = async (date: string) => {
    ...
    try {
      if (!carId || carId === 'undefined') {
        throw new Error('Invalid car ID. Please navigate to this page from a car listing.');
      }
  ...
  if (bookingConfirmed) {
    return (
      <div className="min-h-screen bg-gradient-to-r from-gray-50 to-gray-100 py-20">
      ...
      {[1, 2, 3].map((stepNum, index) => (
        ...
        {step === 1 && (
          <div className="space-y-6">
                  ...
                  {availabilityData.map((slot) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!available}
                        className={`py-3 px-4 rounded-lg font-medium transition relative ${
                      ...
                      >
                        ...
                        {!available && (
                          <FiX className="absolute top-1 right-1 text-red-600" size={16} />
                        )}
                      </button>
                    );
                  })}
            ...
            {selectedDate && !loading && availabilityData.length === 0 && !error && (
